...
// Reset XY velocity states to last GPS measurement if available or to zero if
// in constant position mode or if PV aiding is not absolute Do not reset
// vertical velocity using GPS as there is baro alt available to constrain drift
...
void NavEKF3_core::ResetPosition(resetDataSource posResetSource) {
  ...
  if (PV_AidingMode != AID_ABSOLUTE) {
    ...
    if ((imuSampleTime_ms - lastTimeGpsReceived_ms < 250 &&
      ...
      // calculate position
      const Location gpsloc{gps_corrected.lat, gps_corrected.lng, 0,
                            Location::AltFrame::ABSOLUTE};
      stateStruct.position.xy() = EKF_origin.get_distance_NE_ftype(gpsloc);
      // compensate for offset  between last GPS measurement and the EKF time
      // horizon. Note that this is an unusual time delta in that it can be both
      // -ve and +ve
...
// use the range beacon data as a second preference
      ...
      if (auto *dash = AP_PipeDash::get_singleton()) {
        dash->set("rst.from_n", (float)posResetNE.x);
        dash->set("rst.from_e", (float)posResetNE.y);
        dash->set("rst.to_n", (float)rngBcn.receiverPos.x);
        dash->set("rst.to_e", (float)rngBcn.receiverPos.y);
      }
#endif
...
#if AP_PIPEDASH_ENABLED
  if (auto *dash = AP_PipeDash::get_singleton()) {
    dash->set("res.jump.x", (float)posResetNE.x);
    dash->set("res.jump.y", (float)posResetNE.y);
  }
#endif
}

#if EK3_FEATURE_POSITION_RESET
// Sets the EKF's NE horizontal position states and their corresponding
// variances from a supplied WGS-84 location and optionally uncertainty The
// altitude element of the location is not used. If accuracy is not known should
// be passed as NaN. Returns true if the set was successful
bool NavEKF3_core::setLatLng(const Location &loc, float posAccuracy,
                             uint32_t timestamp_ms) {
  if ((imuSampleTime_ms - lastGpsPosPassTime_ms) <
          frontend->deadReckonDeclare_ms ||
  ...
  // Correct the position for time delay relative to fusion time horizon
  // assuming a constant velocity Limit time stamp to a range between current
  // time and 5 seconds ago
  const uint32_t timeStampConstrained_ms =
      MAX(MIN(timestamp_ms, imuSampleTime_ms), imuSampleTime_ms - 5000);
  const int32_t delta_ms =
      int32_t(imuDataDelayed.time_ms - timeStampConstrained_ms);
  const ftype delaySec = 1E-3F * ftype(delta_ms);
  const Vector2F newPosNE = EKF_origin.get_distance_NE_ftype(loc) +
                            stateStruct.velocity.xy() * delaySec;
...
//    storedOutput, outputDataNew and outputDataDelayed are updated with the
//    change in position lastPosReset_ms is updated with the time of the reset
...
//    storedOutput, outputDataNew and outputDataDelayed are updated with the
//    change in position lastPosResetD_ms is updated with the time of the reset
...
void NavEKF3_core::ResetHeight(void) {
  ...
  // Reset the vertical velocity state using GPS vertical velocity if we are
  // airborne Check that GPS vertical velocity data is available and can be used
  if (inFlight && (gpsIsInUse || badIMUdata) &&
      frontend->sources.useVelZSource(AP_NavEKF_Source::SourceZ::GPS,
                                      core_index) &&
      gpsDataNew.have_vz && (imuSampleTime_ms - gpsDataDelayed.time_ms < 500)) {
...
bool NavEKF3_core::resetHeightDatum(void) {
  ...
  // adjust the height of the EKF origin so that the origin plus baro height
  // before and after the reset is the same
  if (validOrigin) {
    if (!gpsGoodToAlign) {
...
/*
  correct GPS data for position offset of antenna phase centre relative to the
  IMU
 */
void NavEKF3_core::CorrectGPSForAntennaOffset(gps_elements &gps_data) const {
  ...
  const Vector3F posOffsetBody =
      dal.gps().get_antenna_offset(gps_data.sensor_idx).toftype() -
      accelPosOffset;
  if (posOffsetBody.is_zero()) {
    return;
  }

  // TODO use a filtered angular rate with a group delay that matches the GPS
  // delay
  ...
  Location::offset_latlng(gps_data.lat, gps_data.lng, -posOffsetEarth.x,
                          -posOffsetEarth.y);
  gps_data.hgt += posOffsetEarth.z;
}

// correct external navigation earth-frame position using sensor body-frame
// offset
void NavEKF3_core::CorrectExtNavForSensorOffset(
    ext_nav_elements &ext_nav_data) {
  // return immediately if already corrected
  ...
  // external nav data is against the public_origin, so convert to offset from
  // EKF_origin
  ...
  const Vector3F posOffsetBody =
      visual_odom->get_pos_offset().toftype() - accelPosOffset;
  if (posOffsetBody.is_zero()) {
    return;
  }
  Vector3F posOffsetEarth = prevTnb.mul_transpose(posOffsetBody);
...
// correct external navigation earth-frame velocity using sensor body-frame
// offset
void NavEKF3_core::CorrectExtNavVelForSensorOffset(
    ext_nav_vel_elements &ext_nav_vel_data) const {
  // return immediately if already corrected
  ...
  // TODO use a filtered angular rate with a group delay that matches the sensor
  // delay
  const Vector3F angRate =
      imuDataDelayed.delAng * (1.0 / imuDataDelayed.delAngDT);
  ext_nav_vel_data.vel +=
      get_vel_correction_for_sensor_offset(posOffsetBody, prevTnb, angRate);
#endif
}

// calculate velocity variance helper function
void NavEKF3_core::CalculateVelInnovationsAndVariances(
    const Vector3F &velocity, ftype noise, ftype accel_scale,
    Vector3F &innovations, Vector3F &variances) const {
  // innovations are latest estimate - latest observation
  innovations = stateStruct.velocity - velocity;

  const ftype obs_data_chk =
      sq(constrain_ftype(noise, 0.05, 5.0)) + sq(accel_scale * accNavMag);
...
void NavEKF3_core::SelectVelPosFusion() {
  // Check if the magnetometer has been fused on that time step and the filter
  // is running at faster than 200 Hz If so, don't fuse measurements on this
  // time step to reduce frame over-runs Only allow one time slip to prevent
  // high rate magnetometer data preventing fusion of other measurements
  ...
  extNavDataToFuse =
      storedExtNav.recall(extNavDataDelayed, imuDataDelayed.time_ms);
  if (extNavDataToFuse) {
    CorrectExtNavForSensorOffset(extNavDataDelayed);
  }
  extNavVelToFuse =
      storedExtNavVel.recall(extNavVelDelayed, imuDataDelayed.time_ms);
  if (extNavVelToFuse) {
    ...
    CalculateVelInnovationsAndVariances(
        extNavVelDelayed.vel, extNavVelDelayed.err,
        frontend->extNavVelVarAccScale, extNavVelInnov, extNavVelVarInnov);
  ...
  if (gpsDataToFuse) {
    ...
    // calculate innovations and variances for reporting purposes only
    CalculateVelInnovationsAndVariances(
        gpsDataDelayed.vel, frontend->_gpsHorizVelNoise.get(),
        frontend->gpsNEVelVarAccScale, gpsVelInnov, gpsVelVarInnov);
    // record time GPS data was retrieved from the buffer (for timeout checks)
    gpsRetrieveTime_ms = dal.millis();
  }

  // detect position source changes.  Trigger position reset if position source
  // is valid
  const AP_NavEKF_Source::SourceXY posxy_source =
  ...
  if (gpsDataToFuse && (PV_AidingMode == AID_ABSOLUTE) &&
    ...
    fuseVelData = frontend->sources.useVelXYSource(
        AP_NavEKF_Source::SourceXY::GPS, core_index);
    fuseVelVertData = frontend->sources.useVelZSource(
    ...
    const Location gpsloc{gpsDataDelayed.lat, gpsDataDelayed.lng, 0,
                          Location::AltFrame::ABSOLUTE};
  ...
  if (extNavVelToFuse && frontend->sources.useVelXYSource(
                             AP_NavEKF_Source::SourceXY::EXTNAV, core_index)) {
    fuseVelData = true;
    velPosObs[0] = extNavVelDelayed.vel.x;
    velPosObs[1] = extNavVelDelayed.vel.y;
  }
  if (extNavVelToFuse && frontend->sources.useVelZSource(
                             AP_NavEKF_Source::SourceZ::EXTNAV, core_index)) {
    fuseVelVertData = true;
    velPosObs[2] = extNavVelDelayed.vel.z;
  }
#endif

  // we have GPS data to fuse and a request to align the yaw using the GPS
  // course
  if (gpsYawResetRequest) {
    realignYawGPS(false);
  }

  // Select height data to be fused from the available baro, range finder and
  // GPS sources
  selectHeightForFusion();

  // if we are using GPS, check for a change in receiver and reset position and
  // height
  if (gpsDataToFuse && (PV_AidingMode == AID_ABSOLUTE) &&
      (posxy_source == AP_NavEKF_Source::SourceXY::GPS) &&
      (gpsDataDelayed.sensor_idx != last_gps_idx || posxy_source_reset)) {
  ...
  if (extNavDataToFuse && (PV_AidingMode == AID_ABSOLUTE) &&
      (posxy_source == AP_NavEKF_Source::SourceXY::EXTNAV) &&
      (extNavDataDelayed.posReset || posxy_source_reset)) {
    // mark a source reset as consumed
    posxy_source_reset = false;
  ...
  // If we are operating without any aiding, fuse in constant position of
  // constant velocity measurements to constrain tilt drift. This assumes a
  // non-manoeuvring vehicle. Do this to coincide with the height fusion.
  if (fuseHgtData && PV_AidingMode == AID_NONE) {
    if (assume_zero_sideslip() && tiltAlignComplete && motorsArmed) {
      // handle special case where we are launching a FW aircraft without
      // magnetometer
      fusePosData = false;
      velPosObs[0] = 0.0f;
      velPosObs[1] = 0.0f;
      ...
      if (imuDataDelayed.delVel.x >
          1.1f * GRAVITY_MSS * imuDataDelayed.delVelDT) {
        lastLaunchAccelTime_ms = imuSampleTime_ms;
        fuseVelData = false;
        fuseVelVertData = false;
        resetVelNE = true;
      } else if (lastLaunchAccelTime_ms != 0 &&
                 (imuSampleTime_ms - lastLaunchAccelTime_ms) < 10000) {
        fuseVelData = false;
        fuseVelVertData = false;
        resetVelNE = true;
      } else {
...
void NavEKF3_core::FuseVelPosNED() {
  ...
  if (fuseVelData || fuseVelVertData || fusePosData || fuseHgtData) {
    ...
    // Use different errors if operating without external aiding using an
    // assumed position or velocity of zero
    if (PV_AidingMode == AID_NONE) {
      if (tiltAlignComplete && motorsArmed) {
        // This is a compromise between corrections for gyro errors and reducing
        // effect of manoeuvre accelerations on tilt estimate
...
#if EK3_FEATURE_EXTERNAL_NAV
      const bool extNavUsedForVel =
          extNavVelToFuse &&
          frontend->sources.useVelXYSource(AP_NavEKF_Source::SourceXY::EXTNAV,
                                           core_index);
      if (extNavUsedForVel) {
        R_OBS[2] = R_OBS[0] =
            sq(constrain_ftype(extNavVelDelayed.err, 0.05f, 50.0f));
      } else
#endif
          if (gpsSpdAccuracy > 0.0f) {
        // use GPS receivers reported speed accuracy if available and floor at
        // value set by GPS velocity noise parameter
        R_OBS[0] = sq(constrain_ftype(gpsSpdAccuracy,
                                      frontend->_gpsHorizVelNoise, 50.0f));
        R_OBS[2] = sq(
            constrain_ftype(gpsSpdAccuracy, frontend->_gpsVertVelNoise, 50.0f));
      } else {
        // calculate additional error in GPS velocity caused by manoeuvring
        R_OBS[0] =
            sq(constrain_ftype(frontend->_gpsHorizVelNoise, 0.05f, 5.0f)) +
            sq(frontend->gpsNEVelVarAccScale * accNavMag);
        R_OBS[2] =
            sq(constrain_ftype(frontend->_gpsVertVelNoise, 0.05f, 5.0f)) +
            sq(frontend->gpsDVelVarAccScale * accNavMag);
      }
      R_OBS[1] = R_OBS[0];
      // Use GPS reported position accuracy if available and floor at value set
      // by GPS position noise parameter
#if EK3_FEATURE_EXTERNAL_NAV
      if (extNavUsedForPos) {
        ...
        R_OBS[3] = sq(constrain_ftype(gpsPosAccuracy,
                                      frontend->_gpsHorizPosNoise, 100.0f));
      } else {
      ...
      R_OBS[4] = R_OBS[3];
      // For data integrity checks we use the same measurement variances as used
      // to calculate the Kalman gains for all measurements except GPS
      // horizontal velocity For horizontal GPS velocity we don't want the
      // acceptance radius to increase with reported GPS accuracy so we use a
      // value based on best GPS performance plus a margin for manoeuvres. It is
      // better to reject GPS horizontal velocity errors early
      ftype obs_data_chk;
#if EK3_FEATURE_EXTERNAL_NAV
      if (extNavUsedForVel) {
        obs_data_chk = sq(constrain_ftype(extNavVelDelayed.err, 0.05f, 5.0f)) +
                       sq(frontend->extNavVelVarAccScale * accNavMag);
      } else
#endif
      {
        obs_data_chk =
      ...
      R_OBS_DATA_CHECKS[0] = R_OBS_DATA_CHECKS[1] = R_OBS_DATA_CHECKS[2] =
          obs_data_chk;
    }
    ...
    // if vertical GPS velocity data and an independent height source is being
    // used, check to see if the GPS vertical velocity and altimeter innovations
    // have the same sign and are outside limits. If so, then it is likely
    // aliasing is affecting the accelerometers and we should disable the GPS
    // and barometer innovation consistency checks.
    const bool fuse_gps_vz = frontend->sources.useVelZSource(
                                 AP_NavEKF_Source::SourceZ::GPS, core_index) &&
                             gpsDataDelayed.have_vz;
    if (fuse_gps_vz && fuseVelVertData &&
        (frontend->sources.getPosZSource(core_index) !=
      ...
      // bounds Step the test threshold up in stages from 1 to 2 to 3 sigma
      // after exiting from a previous bad IMU event so that a subsequent error
      ...
      if ((hgtErr * velDErr > 0.0f) && (sq(hgtErr) > R_gain * R_OBS[5]) &&
          (sq(velDErr) > R_gain * R_OBS[2])) {
    ...
    if (fusePosData) {
      ...
      // Don't allow test to fail if not navigating and using a constant
      // position assumption to constrain tilt errors because innovations can
      // become large due to vehicle motion.
      ftype maxPosInnov2 =
          sq(MAX(0.01 * (ftype)frontend->_gpsPosInnovGate, 1.0)) *
          (varInnovVelPos[3] + varInnovVelPos[4]);

      posTestRatio = (sq(innovVelPos[3]) + sq(innovVelPos[4])) / maxPosInnov2;
      bool posCheckPassed = false; // boolean true if position measurements have
                                   // passed innovation consistency check
      if (posTestRatio < 1.0f || (PV_AidingMode == AID_NONE)) {
        posCheckPassed = true;
        lastGpsPosPassTime_ms = imuSampleTime_ms;
      } else if ((frontend->_gpsGlitchRadiusMax <= 0) &&
                 (PV_AidingMode != AID_NONE)) {
        // Handle the special case where the glitch radius parameter has been
        // set to a non-positive number. The innovation variance is increased to
        // limit the state update to an amount corresponding to a test ratio
        // of 1.
        lastGpsPosPassTime_ms = imuSampleTime_ms;
      ...
      // Always fuse data if bad IMU to prevent aliasing and clipping pulling
      // the state estimate away from the measurement un-opposed if test
      // threshold is exceeded.
      if (posCheckPassed || posTimeout || badIMUdata) {
        // if timed out or outside the specified uncertainty radius, reset to
        // the external sensor if velocity drift is being constrained, dont
        // reset until gps passes quality checks
        ...
        if ((posTimeout || posVarianceIsTooLarge) &&
          ...
          // Reset the position variances and corresponding covariances to a
          // value that will pass the checks
          zeroRows(P, 7, 8);
          zeroCols(P, 7, 8);
          P[7][7] = sq(ftype(0.5f * frontend->_gpsGlitchRadiusMax));
          P[8][8] = P[7][7];

          // Reset the normalised innovation to avoid failing the bad fusion
          // tests
          posTestRatio = 0.0f;
    ...
    if (fuseVelData) {
      uint8_t imax = 2;
      // Don't fuse vertical velocity observations if disabled in sources or not
      // available
      const bool fuse_extnav_vz =
      ...
      velTestRatio =
          innovVelSumSq /
          (varVelSum * sq(MAX(0.01 * (ftype)frontend->_gpsVelInnovGate, 1.0)));
      bool velCheckPassed = false; // boolean true if velocity measurements have
                                   // passed innovation consistency checks
      if (velTestRatio < 1.0) {
        velCheckPassed = true;
        lastVelPassTime_ms = imuSampleTime_ms;
      } else if (frontend->_gpsGlitchRadiusMax <= 0) {
      ...
      // Use velocity data if healthy, timed out or when IMU fault has been
      // detected Always fuse data if bad IMU to prevent aliasing and clipping
      // pulling the state estimate away from the measurement un-opposed if test
      // threshold is exceeded.
      if (velCheckPassed || velTimeout || badIMUdata) {
        // If we are doing full aiding and velocity fusion times out, reset to
        // the external sensor velocity
    ...
    if (fuseHgtData) {
      ...
      hgtTestRatio = sq(innovVelPos[5]) /
                     (sq(MAX(0.01 * (ftype)frontend->_hgtInnovGate, 1.0)) *
                      varInnovVelPos[5]);

      // When on ground we accept a larger test ratio to allow the filter to
      // handle large switch on IMU bias errors without rejecting the height
      // sensor.
      const bool onGroundNotNavigating =
          (PV_AidingMode == AID_NONE) && onGround;
      const float maxTestRatio = onGroundNotNavigating ? 3.0f : 1.0f;
      bool hgtCheckPassed = false; // boolean true if height measurements have
                                   // passed innovation consistency check
      if (hgtTestRatio < maxTestRatio) {
        hgtCheckPassed = true;
        lastHgtPassTime_ms = imuSampleTime_ms;
      } else if ((frontend->_gpsGlitchRadiusMax <= 0) &&
                 !onGroundNotNavigating &&
      ...
      // Use height data if innovation check passed or timed out or if bad IMU
      // data Always fuse data if bad IMU to prevent aliasing and clipping
      // pulling the state estimate away from the measurement un-opposed if test
      // threshold is exceeded.
      if (hgtCheckPassed || hgtTimeout || badIMUdata) {
        // Calculate a filtered value to be used by pre-flight health checks
        // We need to filter because wind gusts can generate significant baro
        // noise and we want to be able to detect bias errors in the inertial
        // solution
        if (onGround) {
          ftype dtBaro = (imuSampleTime_ms - lastHgtPassTime_ms) * 1.0e-3;
          const ftype hgtInnovFiltTC = 2.0;
          ftype alpha =
              constrain_ftype(dtBaro / (dtBaro + hgtInnovFiltTC), 0.0, 1.0);
    ...
    if (fuseVelData) {
      fuseData[0] = true;
      fuseData[1] = true;
      // currently we do not support independant vertical velocity measurement
      // fuision
      if (fuseVelVertData) {
        fuseData[2] = true;
      }
      ...
      if (fuseData[obsIndex]) {
        ...
        if (obsIndex <= 2) {
          innovVelPos[obsIndex] =
              stateStruct.velocity[obsIndex] - velPosObs[obsIndex];
          ...
          if ((dal.get_touchdown_expected() || dal.get_takeoff_expected()) &&
            ...
            // when baro positive pressure error due to ground effect is
            // expected, floor the barometer innovation at gndBaroInnovFloor
            // constrain the correction between 0 and
            // gndBaroInnovFloor+gndMaxBaroErr this function looks like this:
            //         |/
            //---------|---------
            //    ____/|
            //   /     |
        ...
        // inhibit delta angle bias state estimation by setting Kalman gains to
        // zero
        if (!inhibitDelAngBiasStates) {
          for (uint8_t i = 10; i <= 12; i++) {
            // Don't try to learn gyro bias if not aiding and the axis is
            // less than 45 degrees from vertical because the bias is poorly
            // observable
        ...
        // Inhibit delta velocity bias state estimation by setting Kalman gains
        // to zero Don't use 'fake' horizontal measurements used to constrain
        // attitude drift during periods of non-aiding to learn bias as these
        // can give incorrect esitmates.
        const bool horizInhibit =
            PV_AidingMode == AID_NONE && obsIndex != 2 && obsIndex != 5;
        ...
        // inhibit magnetic field state estimation by setting Kalman gains to
        // zero
        if (!inhibitMagStates) {
        ...
        // Check that we are not going to drive any variances negative and skip
        // the update if so
        ...
        if (healthyFusion) {
          ...
          // variances to prevent ill-conditioning.
          ForceSymmetry();
          ConstrainVariances();
          ...
          for (uint8_t i = 0; i <= stateIndexLim; i++) {
            statesArray[i] =
                statesArray[i] - Kfusion[i] * innovVelPos[obsIndex];
          }
...
// select the height measurement to be fused from the available baro, range
// finder and GPS sources
void NavEKF3_core::selectHeightForFusion() {
  ...
  readRangeFinder();
  rangeDataToFuse =
      storedRange.recall(rangeDataDelayed, imuDataDelayed.time_ms);

  // correct range data for the body frame position offset relative to the IMU
  // the corrected reading is the reading that would have been taken if the
  // sensor was co-located with the IMU
  const auto *_rng = dal.rangefinder();
  if (_rng && rangeDataToFuse) {
    auto *sensor = _rng->get_backend(rangeDataDelayed.sensor_idx);
    if (sensor != nullptr) {
      Vector3F posOffsetBody =
          sensor->get_pos_offset().toftype() - accelPosOffset;
      if (!posOffsetBody.is_zero()) {
        Vector3F posOffsetEarth = prevTnb.mul_transpose(posOffsetBody);
  ...
  if ((frontend->sources.getPosZSource(core_index) ==
    ...
    const ftype rangeMaxUse =
        1e-2 * (ftype)_rng->max_distance_orient(ROTATION_PITCH_270) *
        (ftype)frontend->_useRngSwHgt;
    bool aboveUpperSwHgt =
        (terrainState - stateStruct.position.z) > rangeMaxUse;
    bool belowLowerSwHgt =
        ((terrainState - stateStruct.position.z) < 0.7f * rangeMaxUse) &&
        (imuSampleTime_ms - gndHgtValidTime_ms < 1000);

    // If the terrain height is consistent and we are moving slowly, then it can
    // be used as a height reference in combination with a range finder apply a
    // hysteresis to the speed check to prevent rapid switching
    bool dontTrustTerrain, trustTerrain;
    if (filterStatus.flags.horiz_vel) {
      // We can use the velocity estimate
      ftype horizSpeed = stateStruct.velocity.xy().length();
      dontTrustTerrain =
          (horizSpeed > frontend->_useRngSwSpd) || !terrainHgtStable;
      ftype trust_spd_trigger =
          MAX((frontend->_useRngSwSpd - 1.0f), (frontend->_useRngSwSpd * 0.5f));
    ...
    /*
     * Switch between range finder and primary height source using height above
     * ground and speed thresholds with hysteresis to avoid rapid switching.
     * Using range finder for height requires a consistent terrain height which
     * cannot be assumed if the vehicle is moving horizontally.
     */
    if ((aboveUpperSwHgt || dontTrustTerrain) &&
        (activeHgtSource == AP_NavEKF_Source::SourceZ::RANGEFINDER)) {
    ...
    activeHgtSource = AP_NavEKF_Source::SourceZ::BARO;
             ...
             validOrigin && gpsAccuracyGood) {
    activeHgtSource = AP_NavEKF_Source::SourceZ::GPS;
#if EK3_FEATURE_BEACON_FUSION
  } else if ((frontend->sources.getPosZSource(core_index) ==
              AP_NavEKF_Source::SourceZ::BEACON) &&
             validOrigin && rngBcn.goodToAlign) {
    activeHgtSource = AP_NavEKF_Source::SourceZ::BEACON;
#endif
#if EK3_FEATURE_EXTERNAL_NAV
  } else if ((frontend->sources.getPosZSource(core_index) ==
              AP_NavEKF_Source::SourceZ::EXTNAV) &&
             extNavDataIsFresh) {
    activeHgtSource = AP_NavEKF_Source::SourceZ::EXTNAV;
#endif
  }

  // Use Baro alt as a fallback if we lose range finder, GPS, external nav or
  // Beacon
  bool lostRngHgt =
      ((activeHgtSource == AP_NavEKF_Source::SourceZ::RANGEFINDER) &&
       !rangeFinderDataIsFresh);
  bool lostGpsHgt = ((activeHgtSource == AP_NavEKF_Source::SourceZ::GPS) &&
                     ((imuSampleTime_ms - lastTimeGpsReceived_ms) > 2000 ||
                      !gpsAccuracyGoodForAltitude));
#if EK3_FEATURE_BEACON_FUSION
  bool lostRngBcnHgt =
      ((activeHgtSource == AP_NavEKF_Source::SourceZ::BEACON) &&
       ((imuSampleTime_ms - rngBcn.dataDelayed.time_ms) > 2000));
#endif
  ...
  // if there is new baro data to fuse, calculate filtered baro data required by
  // other processes
  if (baroDataToFuse) {
    // calculate offset to baro data that enables us to switch to Baro height
    // use during operation
    if (activeHgtSource != AP_NavEKF_Source::SourceZ::BARO) {
      calcFiltBaroOffset();
    }
    // filtered baro data used to provide a reference for takeoff
    // it is is reset to last height measurement on disarming in
    // performArmingChecks()
  ...
  // If we are not using GPS as the primary height sensor, correct EKF origin
  // height so that combined local NED position height and origin height remains
  // consistent with the GPS altitude This also enables the GPS height to be
  // used as a backup height source
  if (gpsDataToFuse &&
      (((frontend->_originHgtMode & (1 << 0)) &&
        (activeHgtSource == AP_NavEKF_Source::SourceZ::BARO)) ||
       ((frontend->_originHgtMode & (1 << 1)) &&
        (activeHgtSource == AP_NavEKF_Source::SourceZ::RANGEFINDER)))) {
  ...
  if (extNavDataToFuse &&
    ...
    if (rangeDataToFuse &&
      ...
      if (prevTnb.c.z >= 0.7) {
        ...
        if (frontend->_originHgtMode & (1 << 1) &&
            frontend->_originHgtMode & (1 << 2)) {
        ...
        posDownObsNoise += sq(rangeDataDelayed.rng * frontend->_terrGradMax) *
                           MAX(0.0f, (1.0f - sq(prevTnb.c.z)));
      } else {
      ...
      // enable fusion
      fuseHgtData = true;
      // set the observation noise using receiver reported accuracy or the
      // horizontal noise scaled for typical VDOP/HDOP ratio
      if (gpsHgtAccuracy > 0.0f) {
        posDownObsNoise = sq(constrain_ftype(
            gpsHgtAccuracy, 1.5f * frontend->_gpsHorizPosNoise, 100.0f));
      } else {
        posDownObsNoise = sq(
            constrain_ftype(1.5f * frontend->_gpsHorizPosNoise, 0.1f, 10.0f));
      }
      ...
      // correct sensor so that local position height adjusts to match GPS
      if (frontend->_originHgtMode & (1 << 0) &&
          frontend->_originHgtMode & (1 << 2)) {
        hgtMea += (float)(ekfGpsRefHgt - 0.01 * (double)EKF_origin.alt);
      }
      // enable fusion
      ...
      // set the observation noise
      posDownObsNoise =
          sq(constrain_ftype(frontend->_baroAltNoise, 0.1f, 100.0f));
      // reduce weighting (increase observation noise) on baro if we are likely
      // to be experiencing rotor wash ground interaction
      if (dal.get_takeoff_expected() || dal.get_touchdown_expected()) {
        posDownObsNoise *= frontend->gndEffectBaroScaler;
      }
      velPosObs[5] = -hgtMea;
    } else if ((activeHgtSource == AP_NavEKF_Source::SourceZ::NONE &&
      ...
      velPosObs[5] = -hgtMea;
      if (onGround) {
        // use a typical vertical positoin observation noise when not flying for
        // faster IMU delta velocity bias estimation
        posDownObsNoise = sq(2.0f);
      } else {
        // alow a larger value when flying to accomodate vertical maneouvres
        posDownObsNoise =
            sq(constrain_ftype(frontend->_baroAltNoise, 2.0f, 100.0f));
      }
    } else {
      fuseHgtData = false;
    }
  ...
  // If we haven't fused height data for a while or have bad IMU data, then
  // declare the height data as being timed out set height timeout period based
  // on whether we have vertical GPS velocity available to constrain drift
  hgtRetryTime_ms = ((useGpsVertVel || useExtNavVel) && !velTimeout)
                        ? frontend->hgtRetryTimeMode0_ms
                        : frontend->hgtRetryTimeMode12_ms;
...
/*
 * Fuse body frame velocity measurements using explicit algebraic equations
 * generated with Matlab symbolic toolbox. The script file used to generate
 * these and other equations in this filter can be found here:
...
void NavEKF3_core::FuseBodyVel() {
  ...
  // Fuse X, Y and Z axis measurements sequentially assuming observation errors
  // are uncorrelated
  for (uint8_t obsIndex = 0; obsIndex <= 2; obsIndex++) {

    // calculate relative velocity in sensor frame including the relative motion
    // due to rotation
    ...
    if (imuDataDelayed.delAngDT > 0.001f) {
      bodyVelPred +=
          (imuDataDelayed.delAng * (1.0f / imuDataDelayed.delAngDT)) %
          posOffsetBody;
    }
    ...
    if (obsIndex == 0) {
      ...
      // calculate innovation variance for X axis observation and protect
      // against a badly conditioned calculation
      if (t76 > R_VEL) {
        t77 = 1.0f / t76;
      ...
      Kfusion[0] = t77 * (t29 + P[0][5] * t4 + P[0][4] * t9 - P[0][6] * t11 +
                          P[0][1] * t18 - P[0][2] * t21 + P[0][3] * t24);
      Kfusion[1] = t77 * (t30 + P[1][5] * t4 + P[1][4] * t9 + P[1][0] * t14 -
                          P[1][6] * t11 - P[1][2] * t21 + P[1][3] * t24);
      Kfusion[2] = t77 * (-t78 + P[2][5] * t4 + P[2][4] * t9 + P[2][0] * t14 -
                          P[2][6] * t11 + P[2][1] * t18 + P[2][3] * t24);
      Kfusion[3] = t77 * (t66 + P[3][5] * t4 + P[3][4] * t9 + P[3][0] * t14 -
                          P[3][6] * t11 + P[3][1] * t18 - P[3][2] * t21);
      Kfusion[4] = t77 * (t69 + P[4][5] * t4 + P[4][0] * t14 - P[4][6] * t11 +
                          P[4][1] * t18 - P[4][2] * t21 + P[4][3] * t24);
      Kfusion[5] = t77 * (t32 + P[5][4] * t9 + P[5][0] * t14 - P[5][6] * t11 +
                          P[5][1] * t18 - P[5][2] * t21 + P[5][3] * t24);
      Kfusion[6] = t77 * (-t81 + P[6][5] * t4 + P[6][4] * t9 + P[6][0] * t14 +
                          P[6][1] * t18 - P[6][2] * t21 + P[6][3] * t24);
      Kfusion[7] =
          t77 * (P[7][5] * t4 + P[7][4] * t9 + P[7][0] * t14 - P[7][6] * t11 +
                 P[7][1] * t18 - P[7][2] * t21 + P[7][3] * t24);
      Kfusion[8] =
          t77 * (P[8][5] * t4 + P[8][4] * t9 + P[8][0] * t14 - P[8][6] * t11 +
                 P[8][1] * t18 - P[8][2] * t21 + P[8][3] * t24);
      Kfusion[9] =
          t77 * (P[9][5] * t4 + P[9][4] * t9 + P[9][0] * t14 - P[9][6] * t11 +
                 P[9][1] * t18 - P[9][2] * t21 + P[9][3] * t24);

      if (!inhibitDelAngBiasStates) {
        Kfusion[10] = t77 * (P[10][5] * t4 + P[10][4] * t9 + P[10][0] * t14 -
                             P[10][6] * t11 + P[10][1] * t18 - P[10][2] * t21 +
                             P[10][3] * t24);
        Kfusion[11] = t77 * (P[11][5] * t4 + P[11][4] * t9 + P[11][0] * t14 -
                             P[11][6] * t11 + P[11][1] * t18 - P[11][2] * t21 +
                             P[11][3] * t24);
        Kfusion[12] = t77 * (P[12][5] * t4 + P[12][4] * t9 + P[12][0] * t14 -
                             P[12][6] * t11 + P[12][1] * t18 - P[12][2] * t21 +
                             P[12][3] * t24);
      } else {
        // zero indexes 10 to 12
        zero_range(&Kfusion[0], 10, 12);
      ...
      if (!inhibitDelVelBiasStates && !badIMUdata) {
        for (uint8_t index = 0; index < 3; index++) {
          ...
          if (!dvelBiasAxisInhibit[index]) {
            Kfusion[stateIndex] =
                t77 * (P[stateIndex][5] * t4 + P[stateIndex][4] * t9 +
                       P[stateIndex][0] * t14 - P[stateIndex][6] * t11 +
                       P[stateIndex][1] * t18 - P[stateIndex][2] * t21 +
                       P[stateIndex][3] * t24);
          } else {
      ...
      if (!inhibitMagStates) {
        Kfusion[16] = t77 * (P[16][5] * t4 + P[16][4] * t9 + P[16][0] * t14 -
                             P[16][6] * t11 + P[16][1] * t18 - P[16][2] * t21 +
                             P[16][3] * t24);
        Kfusion[17] = t77 * (P[17][5] * t4 + P[17][4] * t9 + P[17][0] * t14 -
                             P[17][6] * t11 + P[17][1] * t18 - P[17][2] * t21 +
                             P[17][3] * t24);
        Kfusion[18] = t77 * (P[18][5] * t4 + P[18][4] * t9 + P[18][0] * t14 -
                             P[18][6] * t11 + P[18][1] * t18 - P[18][2] * t21 +
                             P[18][3] * t24);
        Kfusion[19] = t77 * (P[19][5] * t4 + P[19][4] * t9 + P[19][0] * t14 -
                             P[19][6] * t11 + P[19][1] * t18 - P[19][2] * t21 +
                             P[19][3] * t24);
        Kfusion[20] = t77 * (P[20][5] * t4 + P[20][4] * t9 + P[20][0] * t14 -
                             P[20][6] * t11 + P[20][1] * t18 - P[20][2] * t21 +
                             P[20][3] * t24);
        Kfusion[21] = t77 * (P[21][5] * t4 + P[21][4] * t9 + P[21][0] * t14 -
                             P[21][6] * t11 + P[21][1] * t18 - P[21][2] * t21 +
                             P[21][3] * t24);
      } else {
        // zero indexes 16 to 21
        zero_range(&Kfusion[0], 16, 21);
      ...
      if (!inhibitWindStates && !treatWindStatesAsTruth) {
        Kfusion[22] = t77 * (P[22][5] * t4 + P[22][4] * t9 + P[22][0] * t14 -
                             P[22][6] * t11 + P[22][1] * t18 - P[22][2] * t21 +
                             P[22][3] * t24);
        Kfusion[23] = t77 * (P[23][5] * t4 + P[23][4] * t9 + P[23][0] * t14 -
                             P[23][6] * t11 + P[23][1] * t18 - P[23][2] * t21 +
                             P[23][3] * t24);
      } else {
        // zero indexes 22 to 23
        zero_range(&Kfusion[0], 22, 23);
      ...
      // calculate innovation variance for Y axis observation and protect
      // against a badly conditioned calculation
      if (t76 > R_VEL) {
        t77 = 1.0f / t76;
      ...
      Kfusion[0] = t77 * (t29 - P[0][4] * t3 + P[0][5] * t8 + P[0][6] * t12 +
                          P[0][1] * t18 + P[0][2] * t22 - P[0][3] * t25);
      Kfusion[1] = t77 * (t30 - P[1][4] * t3 + P[1][5] * t8 + P[1][0] * t15 +
                          P[1][6] * t12 + P[1][2] * t22 - P[1][3] * t25);
      Kfusion[2] = t77 * (t60 - P[2][4] * t3 + P[2][5] * t8 + P[2][0] * t15 +
                          P[2][6] * t12 + P[2][1] * t18 - P[2][3] * t25);
      Kfusion[3] = t77 * (-t90 - P[3][4] * t3 + P[3][5] * t8 + P[3][0] * t15 +
                          P[3][6] * t12 + P[3][1] * t18 + P[3][2] * t22);
      Kfusion[4] = t77 * (-t78 + P[4][5] * t8 + P[4][0] * t15 + P[4][6] * t12 +
                          P[4][1] * t18 + P[4][2] * t22 - P[4][3] * t25);
      Kfusion[5] = t77 * (t69 - P[5][4] * t3 + P[5][0] * t15 + P[5][6] * t12 +
                          P[5][1] * t18 + P[5][2] * t22 - P[5][3] * t25);
      Kfusion[6] = t77 * (t38 - P[6][4] * t3 + P[6][5] * t8 + P[6][0] * t15 +
                          P[6][1] * t18 + P[6][2] * t22 - P[6][3] * t25);
      Kfusion[7] =
          t77 * (-P[7][4] * t3 + P[7][5] * t8 + P[7][0] * t15 + P[7][6] * t12 +
                 P[7][1] * t18 + P[7][2] * t22 - P[7][3] * t25);
      Kfusion[8] =
          t77 * (-P[8][4] * t3 + P[8][5] * t8 + P[8][0] * t15 + P[8][6] * t12 +
                 P[8][1] * t18 + P[8][2] * t22 - P[8][3] * t25);
      Kfusion[9] =
          t77 * (-P[9][4] * t3 + P[9][5] * t8 + P[9][0] * t15 + P[9][6] * t12 +
                 P[9][1] * t18 + P[9][2] * t22 - P[9][3] * t25);

      if (!inhibitDelAngBiasStates) {
        Kfusion[10] = t77 * (-P[10][4] * t3 + P[10][5] * t8 + P[10][0] * t15 +
                             P[10][6] * t12 + P[10][1] * t18 + P[10][2] * t22 -
                             P[10][3] * t25);
        Kfusion[11] = t77 * (-P[11][4] * t3 + P[11][5] * t8 + P[11][0] * t15 +
                             P[11][6] * t12 + P[11][1] * t18 + P[11][2] * t22 -
                             P[11][3] * t25);
        Kfusion[12] = t77 * (-P[12][4] * t3 + P[12][5] * t8 + P[12][0] * t15 +
                             P[12][6] * t12 + P[12][1] * t18 + P[12][2] * t22 -
                             P[12][3] * t25);
      } else {
      ...
      if (!inhibitDelVelBiasStates && !badIMUdata) {
        for (uint8_t index = 0; index < 3; index++) {
          ...
          if (!dvelBiasAxisInhibit[index]) {
            Kfusion[stateIndex] =
                t77 * (-P[stateIndex][4] * t3 + P[stateIndex][5] * t8 +
                       P[stateIndex][0] * t15 + P[stateIndex][6] * t12 +
                       P[stateIndex][1] * t18 + P[stateIndex][2] * t22 -
                       P[stateIndex][3] * t25);
          } else {
            Kfusion[stateIndex] = 0.0f;
          }
      ...
      if (!inhibitMagStates) {
        Kfusion[16] = t77 * (-P[16][4] * t3 + P[16][5] * t8 + P[16][0] * t15 +
                             P[16][6] * t12 + P[16][1] * t18 + P[16][2] * t22 -
                             P[16][3] * t25);
        Kfusion[17] = t77 * (-P[17][4] * t3 + P[17][5] * t8 + P[17][0] * t15 +
                             P[17][6] * t12 + P[17][1] * t18 + P[17][2] * t22 -
                             P[17][3] * t25);
        Kfusion[18] = t77 * (-P[18][4] * t3 + P[18][5] * t8 + P[18][0] * t15 +
                             P[18][6] * t12 + P[18][1] * t18 + P[18][2] * t22 -
                             P[18][3] * t25);
        Kfusion[19] = t77 * (-P[19][4] * t3 + P[19][5] * t8 + P[19][0] * t15 +
                             P[19][6] * t12 + P[19][1] * t18 + P[19][2] * t22 -
                             P[19][3] * t25);
        Kfusion[20] = t77 * (-P[20][4] * t3 + P[20][5] * t8 + P[20][0] * t15 +
                             P[20][6] * t12 + P[20][1] * t18 + P[20][2] * t22 -
                             P[20][3] * t25);
        Kfusion[21] = t77 * (-P[21][4] * t3 + P[21][5] * t8 + P[21][0] * t15 +
                             P[21][6] * t12 + P[21][1] * t18 + P[21][2] * t22 -
                             P[21][3] * t25);
      } else {
        // zero indexes 16 to 21
        zero_range(&Kfusion[0], 16, 21);
      ...
      if (!inhibitWindStates && !treatWindStatesAsTruth) {
        Kfusion[22] = t77 * (-P[22][4] * t3 + P[22][5] * t8 + P[22][0] * t15 +
                             P[22][6] * t12 + P[22][1] * t18 + P[22][2] * t22 -
                             P[22][3] * t25);
        Kfusion[23] = t77 * (-P[23][4] * t3 + P[23][5] * t8 + P[23][0] * t15 +
                             P[23][6] * t12 + P[23][1] * t18 + P[23][2] * t22 -
                             P[23][3] * t25);
      } else {
        // zero indexes 22 to 23
        zero_range(&Kfusion[0], 22, 23);
      ...
      // calculate innovation variance for Z axis observation and protect
      // against a badly conditioned calculation
      if (t76 > R_VEL) {
        t77 = 1.0f / t76;
      ...
      // calculate Kalman gains for X-axis observation
      Kfusion[0] = t77 * (t29 + P[0][4] * t4 + P[0][6] * t9 - P[0][5] * t11 -
                          P[0][1] * t17 + P[0][2] * t20 + P[0][3] * t24);
      Kfusion[1] =
          t77 * (P[1][4] * t4 + P[1][0] * t14 + P[1][6] * t9 - P[1][5] * t11 -
                 P[1][1] * t17 + P[1][2] * t20 + P[1][3] * t24);
      Kfusion[2] = t77 * (t58 + P[2][4] * t4 + P[2][0] * t14 + P[2][6] * t9 -
                          P[2][5] * t11 - P[2][1] * t17 + P[2][3] * t24);
      Kfusion[3] = t77 * (t66 + P[3][4] * t4 + P[3][0] * t14 + P[3][6] * t9 -
                          P[3][5] * t11 - P[3][1] * t17 + P[3][2] * t20);
      Kfusion[4] = t77 * (t31 + P[4][0] * t14 + P[4][6] * t9 - P[4][5] * t11 -
                          P[4][1] * t17 + P[4][2] * t20 + P[4][3] * t24);
      Kfusion[5] = t77 * (-t80 + P[5][4] * t4 + P[5][0] * t14 + P[5][6] * t9 -
                          P[5][1] * t17 + P[5][2] * t20 + P[5][3] * t24);
      Kfusion[6] = t77 * (t69 + P[6][4] * t4 + P[6][0] * t14 - P[6][5] * t11 -
                          P[6][1] * t17 + P[6][2] * t20 + P[6][3] * t24);
      Kfusion[7] =
          t77 * (P[7][4] * t4 + P[7][0] * t14 + P[7][6] * t9 - P[7][5] * t11 -
                 P[7][1] * t17 + P[7][2] * t20 + P[7][3] * t24);
      Kfusion[8] =
          t77 * (P[8][4] * t4 + P[8][0] * t14 + P[8][6] * t9 - P[8][5] * t11 -
                 P[8][1] * t17 + P[8][2] * t20 + P[8][3] * t24);
      Kfusion[9] =
          t77 * (P[9][4] * t4 + P[9][0] * t14 + P[9][6] * t9 - P[9][5] * t11 -
                 P[9][1] * t17 + P[9][2] * t20 + P[9][3] * t24);

      if (!inhibitDelAngBiasStates) {
        Kfusion[10] = t77 * (P[10][4] * t4 + P[10][0] * t14 + P[10][6] * t9 -
                             P[10][5] * t11 - P[10][1] * t17 + P[10][2] * t20 +
                             P[10][3] * t24);
        Kfusion[11] = t77 * (P[11][4] * t4 + P[11][0] * t14 + P[11][6] * t9 -
                             P[11][5] * t11 - P[11][1] * t17 + P[11][2] * t20 +
                             P[11][3] * t24);
        Kfusion[12] = t77 * (P[12][4] * t4 + P[12][0] * t14 + P[12][6] * t9 -
                             P[12][5] * t11 - P[12][1] * t17 + P[12][2] * t20 +
                             P[12][3] * t24);
      } else {
        // zero indexes 10 to 12
        zero_range(&Kfusion[0], 10, 12);
      ...
      if (!inhibitDelVelBiasStates && !badIMUdata) {
        for (uint8_t index = 0; index < 3; index++) {
          ...
          if (!dvelBiasAxisInhibit[index]) {
            Kfusion[stateIndex] =
                t77 * (P[stateIndex][4] * t4 + P[stateIndex][0] * t14 +
                       P[stateIndex][6] * t9 - P[stateIndex][5] * t11 -
                       P[stateIndex][1] * t17 + P[stateIndex][2] * t20 +
                       P[stateIndex][3] * t24);
          } else {
            Kfusion[stateIndex] = 0.0f;
          }
      ...
      if (!inhibitMagStates) {
        Kfusion[16] = t77 * (P[16][4] * t4 + P[16][0] * t14 + P[16][6] * t9 -
                             P[16][5] * t11 - P[16][1] * t17 + P[16][2] * t20 +
                             P[16][3] * t24);
        Kfusion[17] = t77 * (P[17][4] * t4 + P[17][0] * t14 + P[17][6] * t9 -
                             P[17][5] * t11 - P[17][1] * t17 + P[17][2] * t20 +
                             P[17][3] * t24);
        Kfusion[18] = t77 * (P[18][4] * t4 + P[18][0] * t14 + P[18][6] * t9 -
                             P[18][5] * t11 - P[18][1] * t17 + P[18][2] * t20 +
                             P[18][3] * t24);
        Kfusion[19] = t77 * (P[19][4] * t4 + P[19][0] * t14 + P[19][6] * t9 -
                             P[19][5] * t11 - P[19][1] * t17 + P[19][2] * t20 +
                             P[19][3] * t24);
        Kfusion[20] = t77 * (P[20][4] * t4 + P[20][0] * t14 + P[20][6] * t9 -
                             P[20][5] * t11 - P[20][1] * t17 + P[20][2] * t20 +
                             P[20][3] * t24);
        Kfusion[21] = t77 * (P[21][4] * t4 + P[21][0] * t14 + P[21][6] * t9 -
                             P[21][5] * t11 - P[21][1] * t17 + P[21][2] * t20 +
                             P[21][3] * t24);
      } else {
        // zero indexes 16 to 21
        zero_range(&Kfusion[0], 16, 21);
      ...
      if (!inhibitWindStates && !treatWindStatesAsTruth) {
        Kfusion[22] = t77 * (P[22][4] * t4 + P[22][0] * t14 + P[22][6] * t9 -
                             P[22][5] * t11 - P[22][1] * t17 + P[22][2] * t20 +
                             P[22][3] * t24);
        Kfusion[23] = t77 * (P[23][4] * t4 + P[23][0] * t14 + P[23][6] * t9 -
                             P[23][5] * t11 - P[23][1] * t17 + P[23][2] * t20 +
                             P[23][3] * t24);
      } else {
        // zero indexes 22 to 23
        zero_range(&Kfusion[0], 22, 23);
    ...
    bodyVelTestRatio[obsIndex] =
        sq(innovBodyVel[obsIndex]) / (sq(5.0f) * varInnovBodyVel[obsIndex]);
    ...
    if ((bodyVelTestRatio[obsIndex]) < 1.0f) {
      ...
      if (!bodyVelFusionActive) {
        ...
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "EKF3 IMU%u fusing odometry",
                      (unsigned)imu_index);
      }
      ...
      if (healthyFusion) {
        ...
        // force the covariance matrix to be symmetrical and limit the variances
...
void NavEKF3_core::SelectBodyOdomFusion() {
  ...
  // Check for body odometry data (aka visual position delta) at the fusion time
  // horizon
  const bool bodyOdomDataToFuse =
      storedBodyOdm.recall(bodyOdmDataDelayed, imuDataDelayed.time_ms);
  if (bodyOdomDataToFuse &&
      frontend->sources.useVelXYSource(AP_NavEKF_Source::SourceXY::EXTNAV,
                                       core_index)) {
  ...
  const bool wheelOdomDataToFuse =
      storedWheelOdm.recall(wheelOdmDataDelayed, imuDataDelayed.time_ms);
  if (wheelOdomDataToFuse &&
    ...
    if (wheelOdmDataDelayed.delTime > EKF_TARGET_DT) {
      ...
      ftype fwdSpd = wheelOdmDataDelayed.delAng * wheelOdmDataDelayed.radius *
                     (1.0f / wheelOdmDataDelayed.delTime);

      // get the unit vector from the projection of the X axis onto the
      // horizontal
      Vector3F unitVec;
      ...
      // multiply by forward speed to get velocity vector measured by wheel
      // encoders
      Vector3F velNED = unitVec * fwdSpd;

      // This is a hack to enable use of the existing body frame velocity fusion
      // method
