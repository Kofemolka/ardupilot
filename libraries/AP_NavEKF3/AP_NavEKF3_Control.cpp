...
bool NavEKF3_core::readyToUseBodyOdm(void) const {
  ...
  if (!frontend->sources.useVelXYSource(AP_NavEKF_Source::SourceXY::EXTNAV,
    ...
    // exit immediately if sources not configured to fuse external nav or
    // wheel encoders
    return false;
  }
  ...
  // We require stable roll/pitch angles and gyro bias estimates but do not
  // need the yaw angle aligned to use odometry measurements because they are
  // in a body frame of reference
...
// sets the local NED origin using a LLH location (latitude, longitude,
// height) returns false if the origin is already set
...
// sets the local NED origin using a LLH location (latitude, longitude,
// height) returns false is the origin has already been set
bool NavEKF3_core::setOrigin(const Location &loc) {
  ...
  // but we do want to populate the WMM table even if we don't have a GPS at
  // all
...
void NavEKF3_core::checkGyroCalStatus(void) {
  ...
  if (!use_compass() && (yaw_source_last != AP_NavEKF_Source::SourceYaw::GPS) &&
    ...
    // rotate the variances into earth frame and evaluate horizontal terms
    // only as yaw component is poorly observable without a yaw reference
...
void NavEKF3_core::updateFilterStatus(void) {
  ...
  status.flags.takeoff_detected =
      takeOffDetected; // takeoff for optical flow navigation has been
                       // detected
  status.flags.takeoff =
      dal.get_takeoff_expected(); // The EKF has been told to expect takeoff
                                  // is in a ground effect mitigation mode and
                                  // has started the EKF-GSF yaw estimator
  status.flags.touchdown =
      dal.get_touchdown_expected(); // The EKF has been told to detect
                                    // touchdown and is in a ground effect
                                    // mitigation mode
  ...
  // for reporting purposes we report rejecting airspeed after 3s of not
  // fusing when we want to fuse the data
...
void NavEKF3_core::runYawEstimatorCorrection() {
  ...
  if (EKFGSF_run_filterbank) {
    if (gpsDataToFuse) {
      ...
      // after velocity data has been fused the yaw variance estimate will
      // have been refreshed and is used maintain a history of validity
...
// request times out after YAW_RESET_TO_GSF_TIMEOUT_MS if it cannot be
// actioned
